...
const TESTIMONIALS = [
  {
    ...
    name: 'Camila Bertolucci', role: 'Diretora Comercial', company: 'EstacionePark', segment: 'Estacionamento', metric: 'Volume de leads respondidos'
  },
  {
    ...
    name: 'Rafael Sadler', role: 'Sócio-fundador', company: 'Garcia Sadler', segment: 'Imobiliário', metric: 'Visibilidade do pipeline'
  },
  {
    ...
    name: 'Patrícia Marinho', role: 'Gestora de Marketing', company: 'Grupo Marinho', segment: 'Saúde', metric: 'Retorno do investimento'
  }
];
    ...
    <section className="section dark" id="prova">
      <div className="container">
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:10, justifyContent:'center', marginBottom:56 }}>
          {SEGMENTS.map(s => (
            <div key={s.name} style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.borderColor='rgba(30,144,255,.3)'; e.currentTarget.style.color='var(--text-primary)'; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor='var(--border-dark-soft)'; e.currentTarget.style.color='var(--text-secondary)'; }}
            >
              <span style={{ width:17, height:17, color:'#1E90FF', display:'inline-flex', flexShrink:0 }}>{ICONS[s.icon]}</span>
              {s.name}
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          <div className="mono" style={{ fontSize:10.5, letterSpacing:'.12em', textTransform:'uppercase', color:'var(--text-tertiary)', textAlign:'center', marginBottom:20 }}>
        ...
        <div style={{
          background:'var(--bg-card)',
          border:'1px solid var(--border-dark-soft)',
          borderRadius:20, padding:40, marginBottom:48,
          ...
          <div>
            ...
            <p style={{ marginTop:14, color:'var(--text-secondary)', fontSize:16, lineHeight:1.55 }}>
              A <strong style={{color:'var(--text-primary)'}}>Brava Company S.A.</strong> é uma agência de tecnologia B2B sediada em <strong style={{color:'var(--text-primary)'}}>Alphaville, Barueri/SP</strong>. Endereço físico, CNPJ ativo, equipe presencial.
            </p>
            <p style={{ marginTop:14, color:'var(--text-tertiary)', fontSize:14, fontStyle:'italic' }}>
              Sem operação fantasma. Sem desaparecer depois da venda. Você liga, a gente atende.
            </p>
          </div>
          <div style={{ display:'grid', gap:14 }}>
            {[
              { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>, label:'EDIFÍCIO ALPHA PREMIUM', val:'Al. Rio Negro, 503 · 23º Andar / Alphaville, Barueri/SP' },
              { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z"/></svg>, label:'WHATSAPP', val:'(11) 96334-2541' },
              { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>, label:'GOOGLE', val:'5,0 · perfil verificado' },
            ].map((item, i) => (
              <div key={i} style={{ display:'flex', gap:14, alignItems:'flex-start' }}>
                <div style={{ width:38, height:38, borderRadius:10, background:'var(--brava-gradient)', display:'grid', placeItems:'center', color:'#fff', flex:'0 0 38px' }}>{item.icon}</div>
                <div>
                  <div className="mono" style={{ fontSize:10, color:'var(--text-tertiary)', letterSpacing:'.08em' }}>{item.label}</div>
                  <div style={{ marginTop:3, fontSize:14.5, color:'var(--text-primary)' }}>{item.val}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Depoimentos */}
        <h3 className="h3" style={{ textAlign:'center', marginBottom:32, color:'var(--text-primary)' }}>O que dizem nossos clientes</h3>
          ...
          {TESTIMONIALS.map((t, i) => (
            <figure key={i} style={{
              ...
              <figcaption style={{ marginTop:24, paddingTop:18, borderTop:'1px solid var(--border-dark-soft)' }}>
                ...
                <div className="mono" style={{ fontSize:10, marginTop:12, color:'var(--text-tertiary)', letterSpacing:'.08em', textTransform:'uppercase' }}>
